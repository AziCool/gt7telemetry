...
PACKET_MODE = '~'	# default to '~' for easier identification in network captures, but can be set to 'A' or 'B' if needed (see
PACKET_SCHEME = {'A':0xDEADBEAF,'B':0xDEADBEEF,'~':0x55FABB4F} # A,B,~ are all accepted by the game as heartbeat packets, but '~' is the least common in normal telemetry data, so it may be easier to identify in a network capture if needed.
...
def salsa20_dec(dat):	
	...
	# Notice DEADBEAF, not DEADBEEF, 0x55FABB4F
	#iv2 = iv1 ^ 0xDEADBEAF
	#iv2 = iv1 ^ 0xDEADBEEF
	iv2 = iv1 ^ PACKET_SCHEME.get(PACKET_MODE, None)
	IV = bytearray()
...
###########################################
def printAt(str, row=1, column=1, bold=0, underline=0, reverse=0, fg=None, bg=None):
	if fg is None:
		fg = fg_color
...
###########################################
# utility to convert seconds to MM:SS.mmm format
###########################################
...
###########################################
# load car mapping from local `cars.csv` or fallback to remote GitHub URL if not found locally
###########################################
def load_car_map(
	...
	while True:
		try:
			...
			if len(ddata) > 0 and struct.unpack('i', ddata[0x70:0x70+4])[0] > pktid:
				...
				printAt('{:>10}'.format(len(ddata)), 2, 83)					# packet Size
